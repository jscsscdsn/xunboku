...
var MAC = {
  ...
  'Star': {
    'Init': function () {
      ...
      $("#score-list a").hover(function () {
        $("#score-len").attr("placeholder", $("#score-len").attr("class")).removeClass().addClass("s" + $(this).attr("val"));
        $("#score-alt").attr("placeholder", $("#score-alt").text()).text($(this).attr("title"));
      }, function () {
        $("#score-len").removeClass().addClass($("#score-len").attr("placeholder"));
        $("#score-alt").text($("#score-alt").attr("placeholder"));
      });
      $("#score-list a").click(function () {
        MAC.Ajax(maccms.path + '/index.php/ajax/score?mid=' + $('.mac_star').attr('data-mid') + '&id=' + $('.mac_star').attr('data-id') + '&score=' + (parseInt($(this).attr('val')) * 2), 'get', 'json', '', function (r) {
          if (r.code == 1) {
            MAC.Pop.Msg(200, 20, '感谢您的评分', 1000);
            $("#score-len").removeClass().addClass("s" + Math.ceil(r.data.score / 2)).attr("placeholder", "s" + Math.ceil(r.data.score / 2));
            $("#score-alt").attr("placeholder", $("#score-list a").eq(Math.floor(r.data.score / 2)).attr("title")).text($("#score-list a").eq(Math.floor(r.data.score / 2)).attr("title"));
            $("#average-score").text(r.data.score);
            $("#schedule-score").animate({"width":r.data.score*10+"%"},300);
            $("#average-score-box").animate({"left":r.data.score*10+"%"},300);
          } else {
            MAC.Pop.Msg(150, 20, r.msg, 1000);
          }
        }, function () {
          MAC.Pop.Msg(200, 20, '系统异常，请稍后重试', 1000);
        });
      });
    }
  },
  ...
  'History': {
    ...
    'Init': function () {
      ...
      html = '<p><a class="close-his" href="javascript:void(0);" onclick="$(\'.aUnLogin\').click();" target="_self">关闭</a><a class="clear-his" href="javascript:void(0);" onclick="MAC.History.Clear();" target="_self">清空记录</a></p><ul>';
      if (jsondata.length > 0) {
        for ($i = 0; $i < jsondata.length; $i++) {
          var this_array=jsondata[$i].pic.split('@@');
          if(this_array.length>1){
            var this_epname=this_array[1];
          }else{
            var this_epname='高清';
          }
          if(this_array.length>2){
            var this_detail_link=this_array[2];
          }else{
            var this_detail_link=jsondata[$i].link;
          }
          html += '<li><h5><a href="'+this_detail_link+'" class="his_name">' + jsondata[$i].name + '</a></h5><label><a href="' + jsondata[$i].link + '" class="his_part">'+this_epname+'</a></label></li>';
        }
      } else {
        html += '<li class="no-his"><p>暂无播放历史记录...</p></li>';
      }
      html += '</ul>';

      $('.mac_history').html(html);
      ...
      if (jsondata != undefined) {
        ...
        for ($i = 0; $i < this.Json.length; $i++) {
          if ($i <= this.Limit && this.Json[$i]) {
            var old_array=this.Json[$i].pic.split('@@');
            var this_array=pic.split('@@');
            if (old_array[0] != this_array[0]) {
    ...
    'Clear': function () {
      ...
      $('.mac_history ul').html('<li class="no-his"><p>暂无播放历史记录...</p></li>');
    },
  },
